...
class ForEachOp(Op):
    ...
    def run(self, context: Context):
        ...
        source_name = Op.get_parameter(context, self.op_def, 'source', is_required=True, render_expression=True)
        database_name = Op.get_parameter(context, self.op_def, 'database', is_required=False, render_expression=True)
        namespace_name = Op.get_parameter(context, self.op_def, 'namespace', is_required=False, render_expression=True)
        table_name= Op.get_parameter(context, self.op_def, 'table', is_required=True, render_expression=True)
        table_address = TableAddress(database_name, namespace_name, table_name)
        var_name= Op.get_parameter(context, self.op_def, 'as', is_required=True, render_expression=True)
