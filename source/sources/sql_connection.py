...
from source.column import Column
from source.column_schema import ColumnSchema
...
class SQLConnection(Connection):
    ...
    def insert_row(self, row: Row):
        self.conn.execute(self.open_table_for_insert_stmt, row.to_dict())
    ...
    def open_table_for_read(self, table_addr: TableAddress):
        query = text(f"SELECT * FROM {self.source.get_qualified_name(table_addr)}")
